...
let ir3_exp_to_arm offsettbl ir3exp =
  let rec aux ir3exp =
    match ir3exp with
...
let ir3_stmts_to_arm offsettbl ir3stmts =
       ...
       begin
        ...
        ([], [linstr]) :: aux rest   
         | IfStmt3 (condexp, lbl) ->
            ...
            beq .1
            *)
            let (conddata, condinstr, reg) = ir3_exp_to_arm offsettbl condexp in
         ...
         | AssignFieldStmt3 (lhsexp, rhsexp) ->
            (* TODO: Vincent *)
            let (lhsexpdata, lhsexpinstr, reglhs) = ir3_exp_to_arm offsettbl lhsexp in
            let (rhsexpdata, rhsexpinstr, regrhs) = ir3_exp_to_arm offsettbl rhsexp in
...
let ir3_md_to_arm md =
  ...
  let rec pop_tbl count locals =
    ...
    | [] -> ()
    | (_, id) :: rest ->
       Hashtbl.add offsettbl id ((numlocals - count) * 4 + 24);
       pop_tbl (count + 1) rest in
  let _ = pop_tbl 0 md.localvars3 in
  let ldminstr = LDMFD ["v1-v5";"fp";"pc"] in
  let armdata, arminstr = List.split (ir3_stmts_to_arm offsettbl md.ir3stmts) in
  let armdata, arminstr = List.concat armdata, List.concat arminstr in
  (armdata, [ Label md.id3; stminstr; initfp; allocstack ] @ arminstr @ [ldminstr])
